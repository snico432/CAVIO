"""Visual-residual gated CAVIO variant; CAVIO-specific, not in upstream VIFT."""
...
class VisualResidualCAVIOPoseTransformer(nn.Module):
    """
    Gated CAVIO variant with an extra learned visual residual on the IMU stream.

    Each layer keeps the gated cross-attention and FFN residual branches from
    :class:`GatedCAVIOPoseTransformer`, and also adds a direct visual residual
    scaled by ``tanh(alpha_visual_residual)``. Initializing the alpha parameters
    at zero starts from a conservative regime where the added residual is off.
    """
    ...
    def __init__(
        ...
        alpha_ffn_init=0.0,
        alpha_visual_residual_init=0.0,
    ):
        super().__init__()
        ...
        self.alpha_visual_residual = nn.ParameterList(
            [
                nn.Parameter(torch.tensor(float(alpha_visual_residual_init)))
                for _ in range(num_layers)
            ]
        )
    ...
    def forward(self, batch, gt):
        ...
        for idx, layer in enumerate(self.layers):
            ...
            visual_residual = torch.tanh(self.alpha_visual_residual[idx]) * self.residual_dropout(
                visual
            )
            imu = layer["ln1"](
                imu
                + torch.tanh(self.alpha_xattn[idx]) * self.residual_dropout(cross_attn_out)
                + visual_residual
            )
